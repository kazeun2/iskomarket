-- Migration: Allow sender OR receiver to insert transactions
-- Run this in your Supabase SQL editor or with psql as an admin
...
-- Security note:
-- This still requires that the JWT (auth.uid()) exactly matches the sender_id or receiver_id value in the insert.
-- Do not make this policy broader without explicit justification; allowing any authenticated user would be unsafe.
