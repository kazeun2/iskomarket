...
import { MapPin, Heart, MessageCircle, Edit, Check, FileText, X } from 'lucide-react';
...
export function PreviewListingModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="modal-standard modal-pure-white max-w-2xl max-h-[90vh] p-0 overflow-hidden flex flex-col rounded-2xl border border-green-300/40 dark:border-[#14b8a6]/20">
        <DialogHeader className="px-6 py-4 bg-[var(--card)] border-b border-green-200/40 dark:border-emerald-500/20">
          <DialogTitle className="text-[#1A1F1A] dark:text-emerald-300 font-semibold">Preview Your Product</DialogTitle>
          <DialogDescription className="text-gray-600 dark:text-emerald-400/60">
            This is how your product will appear to other users
          </DialogDescription>
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-4 right-4 text-green-600 dark:text-emerald-400 hover:bg-green-100 dark:hover:bg-emerald-500/10 rounded-full"
            onClick={onClose}
            ...
            {formData.images.length > 0 && (
              <div className="relative w-full h-[280px] bg-green-50/30 dark:bg-emerald-950/20 rounded-t-xl overflow-hidden border-b border-green-200/40 dark:border-emerald-500/20">
                ...
                {formData.images.length > 1 && (
                  <div className="absolute top-3 right-3 bg-white backdrop-blur-sm text-green-900 px-2.5 py-1 rounded-full text-xs font-medium">
